...
class MixedState {
public:
  MixedState(const torch::Tensor &data, const std::vector<int64_t> &sys_dim,
  ...
  MixedState expec_state(const std::vector<int64_t> &prob_idx) const;
  ...
  void transform_kraus(const torch::Tensor &list_kraus,
                       const std::vector<int64_t> &sys_idx,
                       bool on_batch = true);
  void transform_many_kraus(const torch::Tensor &list_kraus,
                            const std::vector<std::vector<int64_t>> &sys_idx_list,
                            bool on_batch = true);
  void transform_choi(const torch::Tensor &choi,
                      const std::vector<int64_t> &sys_idx, bool on_batch = true);
  void transform_many_choi(const torch::Tensor &choi,
                           const std::vector<std::vector<int64_t>> &sys_idx_list,
                           bool on_batch = true);

  void add_probability(const torch::Tensor &prob);
  MixedState prob_select(const torch::Tensor &outcome_idx, int64_t prob_idx = -1);
  ...
  MixedState trace(const std::vector<int64_t> &trace_idx);
  MixedState reset(const std::vector<int64_t> &reset_idx,
                   const torch::Tensor &replace_dm);
  MixedState transpose(const std::vector<int64_t> &transpose_idx);
